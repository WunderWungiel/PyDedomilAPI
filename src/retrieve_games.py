...
def retrieve_games(link):
    ...
    if current_page:
        current_page = [int(current_page.group(1)), unicode(response.url)]

        pages_div_match = soup.find('div', {'class': 'PAGES'})
        if pages_div_match:
            last_page_a_match = pages_div_match.find('a', {'class': 'PAGES2'}, text=u"[187][187]")
            ...
            next_page_a_match = pages_div_match.find('a', {'class': 'PAGES2'}, text=u'next[187]')
